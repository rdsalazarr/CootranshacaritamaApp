        ...
        Object.keys(dataPuestos).forEach((clave) => {
            ...
            filas.forEach((elemento) => {
                puestosVehiculo.push({
                    puestoNumero: elemento.puestoNumero,
            ...
            for (let i = 0; i < numeroFilas; i++) {
                ...
                for (let j = 0; j < numeroColumnas; j++) {
                    ...
                    const puestoInfo = distribucionVehiculo.find(info => parseInt(info.tivediid) === numeroPuesto);
                     const puestoInfoArray = distribucionVehiculo.filter(info => parseInt(info.tivedinumero) === numeroPuesto);

                     console.log(puestoInfo,puestoInfoArray[0] );
                     //let id           = numeroPuesto;
                    let id = i * numeroColumnas + j;
                    if (puestoInfo) {
                    //if (puestoInfoArray.length > 0) {
                       // const puestoInfo = puestoInfoArray[0];
                        puestoId  = puestoInfo.tivediid;
                        //id        = puestoInfo.tivedinumero;
                        contenido = puestoInfo.tivedicontenido;
                        clase     = (contenido === 'C') ? 'conductor' : ((contenido === 'P') ? 'pasillo' : 'asiento' );
                        //console.log(puestoId,id,contenido,clase);
                    } else {
                        if (numeroPuesto === 1) {
                            contenido = 'C';
                            clase     = 'conductor';
