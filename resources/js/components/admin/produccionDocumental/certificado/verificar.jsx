...
export default function Verificar(){
    ...
    const modales = [
                        <AnularSolicitarFirma id={modal.data.id } ruta={'certificado'} cerrarModal={cerrarModal} />,
                        <SellarDocumento id={modal.data.id } ruta={'certificado'} cerrarModal={cerrarModal} />,
                        <AnularDocumento id={modal.data.id } ruta={'certificado'} cerrarModal={cerrarModal} />,
                        <VisualizarPdf id={modal.data.id } ruta={'certificado'} />
                    ];
    ...
    const inicio = () =>{
        setLoader(true);
        instance.post('/admin/producion/documental/certificado/list', {tipo:'VERIFICAR'}).then(res=>{
            setData(res.data);
            setLoader(false);
        }) 
    }
