...
export default function Verificar(){
    ...
    const modales = [
                        <AnularSolicitarFirma id={modal.data.id } ruta={'circular'} cerrarModal={cerrarModal} />,
                        <SellarDocumento id={modal.data.id } ruta={'circular'} cerrarModal={cerrarModal} />,
                        <AnularDocumento id={modal.data.id } ruta={'circular'} cerrarModal={cerrarModal} />,
                        <VisualizarPdf id={modal.data.id } ruta={'circular'} />
                    ];
    ...
    const inicio = () =>{
        setLoader(true);
        instance.post('/admin/producion/documental/circular/list', {tipo:'VERIFICAR'}).then(res=>{
            setData(res.data);
            setLoader(false);
        }) 
    }
