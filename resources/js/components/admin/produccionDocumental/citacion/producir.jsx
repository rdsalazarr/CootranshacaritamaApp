...
export default function Producir(){
    ...
    const verificarArea = (area) =>{
        ...
        setMensajeTipoDocumental('Registrar nuevo tipo documental citación del área '+area.depenombre.toLowerCase());  
        setAccion('N');
    }
    ...
    const inicio = () =>{
        setLoader(true);
        instance.post('/admin/producion/documental/citacion/list', {tipo:'PRODUCIR'}).then(res=>{
            setData(res.data);
            setLoader(false);
        })
        setAccion('L');
    ...
    return (
        <Box>
            ...
            : ((accion === 'N') ?
                ...
                :  
                <Box>
                    <NewEdit tipo={'U'} id={idDocumento} ruta='P' volver={inicio}  mensaje={'Editar tipo documental citación'} />
                </Box>
                )
            }
