...
export default function Verificar(){
    ...
    const modales = [
                        <AnularSolicitarFirma id={modal.data.id } ruta={'citacion'} cerrarModal={cerrarModal} />,
                        <SellarDocumento id={modal.data.id } ruta={'citacion'} cerrarModal={cerrarModal} />,
                        <AnularDocumento id={modal.data.id } ruta={'citacion'} cerrarModal={cerrarModal} />,
                        <VisualizarPdf id={modal.data.id } ruta={'citacion'} />
                    ];
    ...
    const inicio = () =>{
        setLoader(true);
        instance.post('/admin/producion/documental/citacion/list', {tipo:'VERIFICAR'}).then(res=>{
            setData(res.data);
            setLoader(false);
        }) 
    }
