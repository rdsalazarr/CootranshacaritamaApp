...
export default function Producir(){
    ...
    const verificarArea = (area) =>{
        ...
        setMensajeTipoDocumental('Registrar nuevo tipo documental constancia del área '+area.depenombre.toLowerCase());
        setAccion('N');
    }

    const modales = [
                        <VerificarArea cerrarModal={cerrarModal} verificarArea={verificarArea} ruta={'constancia'} />,
                        <SolicitarFirma id={(tipo !== 0) ? modal.data.id : null} ruta={'constancia'} cerrarModal={cerrarModal} />,
                        <VisualizarPdf id={(tipo !== 0) ? modal.data.id : null} ruta={'constancia'} />
                    ];
    ...
    const inicio = () =>{
        setLoader(true);
        instance.post('/admin/producion/documental/constancia/list', {tipo:'PRODUCIR'}).then(res=>{
            setData(res.data);
            setLoader(false);
        })
        setAccion('L');
    ...
    return (
        <Box>
            ...
            : ((accion === 'N') ?
                ...
                :  
                <Box>
                    <NewEdit tipo={'U'} id={idDocumento} ruta='P' volver={inicio}  mensaje={'Editar tipo documental constancia'} />
                </Box>
                )
            }
