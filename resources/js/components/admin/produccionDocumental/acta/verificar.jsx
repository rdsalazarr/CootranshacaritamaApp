...
export default function Verificar(){
    ...
    const modales = [
                        <AnularSolicitarFirma id={modal.data.id } ruta={'acta'} cerrarModal={cerrarModal} />,
                        <SellarDocumento id={modal.data.id } ruta={'acta'} cerrarModal={cerrarModal} />,
                        <AnularDocumento id={modal.data.id } ruta={'acta'} cerrarModal={cerrarModal} />,
                        <VisualizarPdf id={modal.data.id } ruta={'acta'} />
                    ];
    ...
    const inicio = () =>{
        setLoader(true);
        instance.post('/admin/producion/documental/acta/list', {tipo:'VERIFICAR'}).then(res=>{
            setData(res.data);
            setLoader(false);
        }) 
    }
    ...
    return (
        <Box>
            <Box sx={{maxHeight: '35em', overflow:'auto'}} sm={{maxHeight: '35em', overflow:'auto'}}>
                <TablaGeneral
                    datos={data}
                    titulo={['Consecutivo', 'Dependencia','Fecha','Hora acta','Dirigido','Estado','Anular firma', 'Sellar', 'Anular documento','PDF']}
